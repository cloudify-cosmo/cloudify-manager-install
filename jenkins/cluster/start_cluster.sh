...
function run_queue1(){
  ...
  sed -e "s/CONTAINER_IP/${QUEUE1_IP}/g" \
      ...
      queue1_config.yaml > rendered_queue1_config.yaml
  cat rendered_queue1_config.yaml
  docker run -d \
    --name ${NODE1_NAME} \
    --network net1 --ip ${QUEUE1_IP} \
    -v $(pwd)/rendered_queue1_config.yaml:/etc/cloudify/config.yaml \
  ...
  docker cp rendered_queue1_config.yaml ${NODE1_NAME}:/etc/cloudify/queue_config.yaml
}
...
function run_queue2(){
  ...
  sed -e "s/CONTAINER_IP/${QUEUE2_IP}/g" \
      ...
      -e "s/QUEUE3_IP/${QUEUE3_IP}/" \
      queue2_config.yaml > rendered_queue2_config.yaml
  cat rendered_queue2_config.yaml
  docker run -d \
    --name ${NODE2_NAME} \
    --network net1 --ip ${QUEUE2_IP} \
    -v $(pwd)/rendered_queue2_config.yaml:/etc/cloudify/config.yaml \
  ...
  docker cp rendered_queue2_config.yaml ${NODE2_NAME}:/etc/cloudify/queue_config.yaml
}
...
function run_queue3(){
  ...
  sed -e "s/CONTAINER_IP/${QUEUE3_IP}/g" \
      -e "s/QUEUE1_IP/${QUEUE1_IP}/" \
      -e "s/QUEUE2_IP/${QUEUE2_IP}/" \
      queue3_config.yaml > rendered_queue3_config.yaml
  cat rendered_queue3_config.yaml
  docker run -d \
     --name ${NODE3_NAME} \
     --network net1 --ip ${QUEUE3_IP} \
     -v $(pwd)/rendered_queue3_config.yaml:/etc/cloudify/config.yaml \
  ...
  docker cp rendered_queue3_config.yaml ${NODE3_NAME}:/etc/cloudify/queue_config.yaml
}
...
function run_db1(){
  ...
  sed -e "s/CONTAINER_IP/${DB1_IP}/g" \
      ...
      -e "s/DB2_IP/${DB2_IP}/g" \
      -e "s/DB3_IP/${DB3_IP}/g" \
      db_config.yaml > rendered_db1_config.yaml
  cat rendered_db1_config.yaml
  sudo docker cp rendered_db1_config.yaml ${NODE1_NAME}:/etc/cloudify/db_config.yaml
...
function run_db2(){
  ...
  sed -e "s/CONTAINER_IP/${DB2_IP}/g" \
      ...
      -e "s/DB2_IP/${DB2_IP}/g" \
      -e "s/DB3_IP/${DB3_IP}/g" \
      db_config.yaml > rendered_db2_config.yaml
  cat rendered_db2_config.yaml
  sudo docker cp rendered_db2_config.yaml ${NODE2_NAME}:/etc/cloudify/db_config.yaml
...
function run_db3(){
  ...
  sed -e "s/CONTAINER_IP/${DB3_IP}/g" \
      ...
      -e "s/DB2_IP/${DB2_IP}/g" \
      -e "s/DB3_IP/${DB3_IP}/g" \
      db_config.yaml > rendered_db3_config.yaml
  cat rendered_db3_config.yaml
  sudo docker cp rendered_db3_config.yaml ${NODE3_NAME}:/etc/cloudify/db_config.yaml
...
function run_manager1(){
  ...
  sed -e "s/CONTAINER_IP/${MANAGER1_IP}/g" \
      ...
      -e "s/DB2_IP/${DB2_IP}/g" \
      -e "s/DB3_IP/${DB3_IP}/g" \
      manager1_config.yaml > rendered_manager1_config.yaml
  cat rendered_manager1_config.yaml
    # Generate ca encrypted key
  openssl rsa -aes256 -passout pass:secret_ca_password -in ca.key -out ca.encrypted.key
  sudo docker cp rendered_manager1_config.yaml ${NODE1_NAME}:/etc/cloudify/manager_config.yaml
...
function run_manager2(){
  ...
  sed -e "s/CONTAINER_IP/${MANAGER2_IP}/g" \
      ...
      -e "s/DB2_IP/${DB2_IP}/g" \
      -e "s/DB3_IP/${DB3_IP}/g" \
      manager2_config.yaml > rendered_manager2_config.yaml
  cat rendered_manager2_config.yaml
  sudo docker cp rendered_manager2_config.yaml ${NODE2_NAME}:/etc/cloudify/manager_config.yaml
...
function run_manager3(){
  ...
  sed -e "s/CONTAINER_IP/${MANAGER3_IP}/g" \
      ...
      -e "s/DB2_IP/${DB2_IP}/g" \
      -e "s/DB3_IP/${DB3_IP}/g" \
      manager3_config.yaml > rendered_manager3_config.yaml
  cat rendered_manager3_config.yaml
  sudo docker cp rendered_manager3_config.yaml ${NODE3_NAME}:/etc/cloudify/manager_config.yaml
